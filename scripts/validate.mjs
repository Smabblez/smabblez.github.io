...
check(styles.includes('--nose-x: 43.8%;\n  --nose-y: 45.4%;\n  --nose-size: 15%;') && styles.includes('--nose-x: 43.8%;\n  --nose-y: 54.2%;\n  --nose-size: 14.5%;') && index.includes('styles.css?v=20260726c') && !styles.includes('honk-label'), 'Hero nose hit frame and stylesheet cache key must stay aligned with both rendered emotes.');
...
check(scriptSource.includes("document.addEventListener('click', (event) => {\n  if (!body.classList.contains('chaos-on')") && !scriptSource.includes("document.addEventListener('pointerdown', (event) => {\n  if (!body.classList.contains('chaos-on')") && styles.includes('-webkit-touch-callout: none;'), 'Chaos page reactions must wait for completed clicks and suppress text-selection callouts.');
