...
check(styles.includes('--nose-x: 43.8%;\n  --nose-y: 45.4%;\n  --nose-size: 15%;') && styles.includes('--nose-x: 43.8%;\n  --nose-y: 54.2%;\n  --nose-size: 14.5%;') && index.includes('styles.css?v=20260726f') && !styles.includes('honk-label'), 'Hero nose hit frame and stylesheet cache key must stay aligned with both rendered emotes.');
check(index.includes('data-cue-deck') && (index.match(/data-show-cue="/g) || []).length === 4 && index.includes('data-cue-status') && index.includes('aria-live="polite"'), 'Homepage audience-control preview must expose four accessible show cues and live feedback.');
check(scriptSource.includes('const cueResponses = {') && scriptSource.includes("cueDeck.dataset.activeCue = cue") && scriptSource.includes("dropChaosSticker(x, y, response.sticker)"), 'Audience-control preview behavior is incomplete.');
