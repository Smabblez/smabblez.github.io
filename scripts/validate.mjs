...
check(index.includes('data-twitch-offline') && index.includes('Recent broadcasts') && index.includes('Best clips'), 'Twitch offline recovery panel is missing.');
...
check(scriptSource.includes('window.Twitch.Player.ONLINE') && scriptSource.includes('window.Twitch.Player.OFFLINE') && scriptSource.includes("setTwitchPlayerState('fallback')"), 'Twitch player must react to live, offline, and unavailable states.');
...
check(styles.includes('--nose-x: 43.8%;\n  --nose-y: 45.4%;\n  --nose-size: 15%;') && styles.includes('--nose-x: 43.8%;\n  --nose-y: 54.2%;\n  --nose-size: 14.5%;') && index.includes('styles.css?v=20260729a') && !styles.includes('honk-label'), 'Hero nose hit frame and stylesheet cache key must stay aligned with both rendered emotes.');
